...
在证明定理\cref{thm:NLS-inverse-x}之前, 我们先建立离散散射数据 $ \{\zeta_{k}, \bar{\zeta}_{k}, a_{k}, b_{k}, \bar{a}_{k}, \bar{b}_{k}\}(1\leq k \leq N) $ 和特征值问题 $ Y_{x} = MY $ 及其伴随问题 $ K_{x} = -KM $ 中的离散特征值的联系, 不妨设
\begin{align}
...
\end{equation}
利用上面渐近行为, 以及对应特征问题 \eqref{eq:reverse-x-NLS-eigenvalue}-\eqref{eq:reverse-x-NLS-adjoint-eigenvalue}, 我们可以利用对称关系求解\cref{thm:NLS-inverse-x}.
...
\begin{proof}
    ...
    \end{equation}
    由此可得, 若 $ \zeta_{k} \in \mathbb{C}_{+} $ 是特征值问题 \eqref{eq:reverse-x-NLS-eigenvalue} 的特征值, 则 $ \hat{\zeta}_{k} = - \zeta_{k}^{*} \in \mathbb{C}_{+} $ 也是其特征值. 另外将 $(\hat{Y}_{k}, \hat{\zeta}_{k})$ 带入 \eqref{eq:large-x-asymptotics-of-reverse-x-NLS-eigenfunction} 可得 $x \to -\infty$ 时
    \begin{equation}
...
\end{theorem}
\begin{proof}
    逆时空 NLS 方程 \eqref{eq:reverse-x-t-NLS} 是从耦合NLS 方程 \eqref{eq:coupled-NLS} 在约化条件\eqref{eq:reverse-x-t-NLS-reduction} 下导出的, 位势矩阵 $ Q_{0} $ 为
    \begin{equation}
        Q_{0} = \begin{pmatrix}
            0        & q(x,0) \\
            -q(-x,0) & 0
