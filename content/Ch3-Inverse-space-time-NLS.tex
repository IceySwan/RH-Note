...
一个问题是为何 RH 问题求解 $P^{+} v_{k} = 0$ 得到的向量 $v_{k}$ 和特征值问题 $Y_{x} = MY$ 的向量相同, 这一点可以参考 Ablowitz 1974 年论文的第二小节, 此处不做证明. 但是在下一章节 ($3 \times 3$ AKNS 系统) 给出详细证明.
\end{problem}
\begin{proof}
...
\begin{equation}
    ...
    \end{aligned}
    \label{ch3-eq-reverse-x-phi-linear-combination}
\end{equation}
其中
\begin{equation}
    \begin{aligned}
...
\begin{equation}
    \alpha_{k}' \phi_{+,1} + \beta_{k}' \phi_{+,2} + \gamma_{k}' \phi_{-,3} = 0
    \label{ch3-eq-reverse-x-phi-linear-combination-2}
\end{equation}
故 \eqref{ch3-eq-reverse-x-phi-linear-combination} 与 \eqref{ch3-eq-reverse-x-phi-linear-combination-2} 等价, 故 $ v_{k0} = c v_{k0}'$, 不失一般性, 可令 $c=1$, 则 $\alpha_{k} = \alpha_{k}', \beta_{k} = \beta_{k}', \gamma_{k} = \gamma_{k}'$, 也即 RH 方法得到的孤子解与特征函数对称性得到的孤子解等价.
