...
\begin{theorem}{Plemelj 定理}
    ...
    \begin{align}
        F_{+} & = \lim_{\substack{z' \to z \\ z' \in \Sigma_{+}}} F(z') = F(z) + \frac{1}{2}f(z) \label{ch1-eq-Plemelj-1}\\
        F_{-} & = \lim_{\substack{z' \to z \\ z' \in \Sigma_{-}}} F(z') = F(z) - \frac{1}{2}f(z)\label{ch1-eq-Plemelj-2}
    \end{align}
\end{theorem}
\begin{proof}
    首先证明闭区线情形:
...
\begin{remark}
    由 RH 问题解的唯一性, RH 问题 \eqref{ch1-eq-RH-analytical-condition} - \eqref{ch1-eq-RH-Matrix-asymptotic-condition} 的解与跳跃矩阵 $ v(z) $ 的分解无关, 因此可以考虑 $ v(z) $ 的平凡解
    \begin{equation}
        b_{-} = I, \quad b_{+} = v
    \end{equation}
    故可构造
...
考虑常系数齐次线性方程组
\begin{equation}
    \frac{dy}{dx} = Ay \label{ch1-ch1-eq-A-linear-system}
\end{equation}
\begin{corollary}
    ...
    \end{equation}
    为 \eqref{ch1-ch1-eq-A-linear-system} 的基解矩阵. 现考虑如下偏微分方程组
    \begin{align}
        \frac{\partial y}{\partial x} & = A y \label{ch1-ch1-eq-AB-linear-system-x} \\
        \frac{\partial y}{\partial t} & = By \label{ch1-ch1-eq-AB-linear-system-t}
    \end{align}
    则由 \eqref{ch1-ch1-eq-AB-linear-system-x} 得 $ y = e^{Ax} c(t) $, 其中 $ c(t) $ 为与 $ x $ 无关的列向量. 将其带入 \eqref{ch1-ch1-eq-AB-linear-system-t}, 可得 $ e^{Ax} c(t) = B e^{A x} c(t) $, 由于 $ AB = BA \implies B e^{A x} = e^{A x} B \implies $

    故 $ c(t) = e^{Bt} c_{0} $, 其中 $ c_{0} $ 为常列向量. 故偏微分方程组 \eqref{ch1-ch1-eq-AB-linear-system-x} - \eqref{ch1-ch1-eq-AB-linear-system-t} 的解为
