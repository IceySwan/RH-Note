...
\end{theorem}
\begin{proof}
	显然 $ f(Z) $ 在 $ D_{1} + D_{2} + \Gamma $ 上连续, 根据 Morera 定理, 只需证明 $ f(z) $ 沿任何闭曲线 $ \sum $ 的积分为 $ 0 $.
...
只需两边取对数变换则有
\begin{equation}
	\ln{G_{+}(z)}- \ln{G_{-}(z)} =  \ln{v(z)}, \quad z\in \sum
\end{equation}
由 Plemelj 公式, 则有
\begin{equation}
	\ln{G(z)} = A + \frac{1}{2 \pi \rmi} \int_{\sum} \frac{v(\xi)}{\xi - z} \rmd \xi \implies G(z) = B \exp{\left( \frac{1}{2 \pi \rmi} \int_{\sum} \frac{f(\xi)}{\xi - z} \rmd \xi \right)}, \quad z\in \sum
\end{equation}
其中 $ B $ 为任意常数.
