...
\begin{proof}
	...
	如果 $ \Sigma \subset D_{1} $, 或 $ \Sigma \subset D_{1} $, 则由 $ f(z) $ 的解析性可得
	\begin{equation}
...
\begin{theorem}{Plemelj 定理}
	...
	\begin{align}
		F_{+} & = \lim_{\substack{z' \to z \\ z' \in \Sigma_{+}}} F(z') = F(z) + \frac{1}{2}f(z) \label{eq:Plemelj-1}\\
		F_{-} & = \lim_{\substack{z' \to z \\ z' \in \Sigma_{-}}} F(z') = F(z) - \frac{1}{2}f(z)\label{eq:Plemelj-2}
	\end{align}
\end{theorem}
\begin{proof}
	首先证明闭区线情形:
	\begin{equation}
		\begin{aligned}
			F_{+} & = \lim_{\substack{z' \to z \newline z' \in \Sigma_{+}}} F(z') = \lim_{z' - z} \int_{\Sigma} \frac{1}{2 \pi \rmi} \frac{f(\xi)}{\xi - z'} \, \rmd \xi                      \\
...
\begin{definition}{RH 问题}
	...
	\begin{align}
		M(z) \in C, \quad ( \mathbb{C}\setminus  \Sigma ) \label{eq:RH-analytical-condition} \\
		M_{+}(z) = M_{-}(z)v(z), \quad z\in \Sigma \label{eq:RH-Matrix-jump-condition}       \\
...
进一步可定义 Cauchy 投影算子
\begin{equation}
	(C_{\pm} f)(z) = \lim_{\substack{z' \to z \in \Sigma \\ z' \in \pm \Sigma}} \frac{1}{2 \pi \rmi} \int_{\Sigma} \frac{f(\xi)}{\xi - z'} \rmd \xi \label{eq:Cauchy-projection}
\end{equation}
...
\begin{theorem}
	...
	则
	\begin{equation}
		M(z) = I + \frac{1}{2 \pi \rmi} \int_{\Sigma} \frac{\mu(\xi) w(\xi)}{\xi - z} \rmd \xi \label{eq:RH-singular-integral-matrix}
	\end{equation}
...
\begin{proof}
	...
	\begin{equation}
		\begin{aligned}
			M_{+} & = I + \lim_{z' \to z \in \Sigma} \frac{1}{2 \pi \rmi }\int_{\Sigma} \frac{\mu(\xi) w(\xi)}{\xi - z} \rmd \xi \overset{\eqref{eq:Cauchy-projection}}{=} I + C_{+}(\mu w)  \\
...
\begin{remark}
	...
	则 RH 问题的解可表示为
	\begin{equation}
		M(z) = I + \frac{1}{2 \pi \rmi} \int_{+\Sigma} \frac{\mu(\xi) (v(\xi) - I)}{\xi - z}  \rmd \xi \label{eq:RH-solution}
	\end{equation}
\end{remark}
...
\begin{remark}
	...
	这很容易通过表达式 \eqref{eq:RH-solution} 看出, 我们将积分路径分解为 $ \Sigma = \Sigma_{1} + \Sigma_{2} $ 且
	\begin{equation}
		v = \begin{cases}
			v_1 \neq I \quad \Sigma_{1}, \\
			v_1 = I \quad \Sigma_{2}
