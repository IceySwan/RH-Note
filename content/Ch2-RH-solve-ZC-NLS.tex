\chapter{RH 方法求解零边界的 NLS 方程}\label{ch2-ZC-NLS}
本章节我们以聚焦 NLS 方程为例, 介绍 RH 方法求解零边界非线性偏微分方程的基本步骤. 主要参考复旦大学范恩贵老师的讲义 \cite{Fan-RH} 的第五章.
\section{聚焦 NLS 方程}
...
考虑聚焦 NLS 方程
\begin{equation}
    \rmi q_{t}(x,t) + q_{xx}(x,t) + 2q(x,t)|q(x,t)|^{2} = 0 \label{ch2-eq-Focus-NLS}
\end{equation}
满足的初值问题 $q(x,0) = q_{0}(x) \in \mathcal{S}(\mathbb{R})$, 其中 $ \mathcal{S}(\mathbb{R}) $ 表示 Schwartz 空间.

方程 \eqref{ch2-eq-Focus-NLS} 具有如下矩阵形式的 Lax 对
\begin{align}
    \psi_{x} & = -\rmi z \sigma_{3} \psi + P \psi \label{ch2-eq-Focus-NLS-Lax-x}     \\
    \psi_{t} & = -2 \rmi z^2 \sigma_{3} \psi + Q \psi \label{ch2-eq-Focus-NLS-Lax-t}
\end{align}
其中
\begin{equation*}
    P = \begin{pmatrix}
...
由于 $ q_{0}(x) \in \mathcal{S}(\mathbb{R}) $, 且 $ q(x,t), q_{x}(x,t) \to 0, (x \to \infty) $, 故 x 足够大时, $ P, Q $ 可忽略, 从而 Lax 对 \eqref{ch2-eq-Focus-NLS-Lax-x} - \eqref{ch2-eq-Focus-NLS-Lax-t} 可近似为
\begin{equation*}
...
\begin{equation}
    % 这里有问题!!!
    E^{-1} = e^{-\rmi \theta(z) \sigma_{3}}, \quad \mu(x,t,z) \to I( x \to \infty) \label{ch2-eq-mu-asymptomatic}
\end{equation}
则可得
\begin{align}
...
\end{align}
带入 Lax 对 \eqref{ch2-eq-Focus-NLS-Lax-x} - \eqref{ch2-eq-Focus-NLS-Lax-t} 可得
\begin{align}
...
故有
\begin{equation}
    \mu_{x} = P\mu -\rmi z [\sigma_{3}, \mu] \implies \mu_{x} + \rmi z [\sigma_{3}, \mu] = P \mu \label{ch2-eq-Focus-NLS-Lax-x-Asymptotic}
\end{equation}
同理可得
\begin{equation}
    \mu_{t} + 2 \rmi z^2 [\sigma_{3}, \mu] = Q \mu \label{ch2-eq-Focus-NLS-Lax-t-Asymptotic}
\end{equation}
由 \eqref{ch2-eq-Focus-NLS-Lax-x-Asymptotic} 式可得
\begin{align}
...
易得 Lax 对的全微分形式
\begin{equation}
    \rmd (\hat{E} \mu) = \hat{E} [ (P \rmd x + Q \rmd t) \mu ] \label{ch2-eq-diffision-mu}
\end{equation}
...
\begin{proof}
    ...
    \begin{equation}
        \mu = \mu^{(0)} + \frac{1}{z} \mu^{(1)} + \cdots = \mu^{(0)} + O(z^{-1}) \label{ch2-eq-mu-Taylor}
    \end{equation}
    其中 $ \mu^{(0)}, \mu^{(1)} $ 与 $ z $ 无关, 将上式代入 \eqref{ch2-eq-Focus-NLS-Lax-x-Asymptotic}, \eqref{ch2-eq-Focus-NLS-Lax-t-Asymptotic} 比较 $ z $ 的次数可得
    \begin{equation}
        \begin{aligned}
    ...
    \begin{equation}
        \begin{aligned}
            ...
            z:     & \quad \rmi [\sigma_{3}, \mu^{(1)}] =  P \mu^{(0)} \label{ch2-eq-mu-Taylor-t-z}
        \end{aligned}
        \implies \mu^{(0)}_x = 0
    \end{equation}
    故 $ \mu^{(0)} $ 为与 $ x $ 无关的对角矩阵, 因此 \eqref{ch2-eq-mu-Taylor} 式对 $ x, z $ 同时取极限, 并交换极限顺序可得
    \begin{equation}
        \lim_{z \to \infty} \lim_{x \to \infty} \mu = \lim_{x \to \infty} \lim_{z \to \infty} \left( \mu^{(0)} + O(z^{(-1)}) \right)
    \end{equation}
    利用 \eqref{ch2-eq-mu-asymptomatic}, \eqref{ch2-eq-mu-Taylor} 式可得 $ \mu^{(0)} = I $, 故 $ \mu \to I (z \to \infty) $.
\end{proof}
再将 $ \mu^{(0)} = I $ 带入 \eqref{ch2-eq-mu-Taylor-t-z} 式比较矩阵对角元素得
\begin{equation}
...
\section{解析性与对称性}\label{ch2-sec-analytic-symmetry}
由于 \eqref{ch2-eq-diffision-mu} 为全微分形式, 积分与路径无关, 故选择两个特殊路径:
\begin{equation*}
    (- \infty, t) \to(x,t) , \quad (+ \infty, t) \to(x,t)
\end{equation*}
因此可获得 Lax 对 \eqref{ch2-eq-Focus-NLS-Lax-x-Asymptotic}, \eqref{ch2-eq-Focus-NLS-Lax-t-Asymptotic} 的两个特征函数
\begin{equation}
    \mu_{1} = I + \int^{x}_{-\infty} e^{- \rmi z (x - y) \sigma_{3}} P \mu_{1}\rmd x, \quad \mu_{2} = I - \int^{x}_{+\infty} e^{- \rmi z (x - y) \sigma_{3}} P \mu_{2}\rmd x \label{ch2-eq-integral-mu}
\end{equation}
\begin{proof}
    由 \eqref{ch2-eq-diffision-mu} 式, 由 $ (\hat{E} \mu)_{x} = \hat{E} P \mu $ 可得
    \begin{subequations}
        \begin{align}
...
\begin{equation}
    \mu_{1}(x,t,z) = \mu_{2}(x,t,z) \hat{E} S(z), \label{ch2-eq-eig-mu-relationship} \quad
    S(z) =
    \begin{pmatrix}
...
\begin{equation}
    \det(\mu_{j}) = \lim_{|x| \to \infty} \det(\mu_{j}) = 1, \quad(j = 1,2) \label{ch2-eq-mu-det}
\end{equation}
故对 \eqref{ch2-eq-eig-mu-relationship} 两边取行列式有 $ \det S(z) = 1 $.
...
\begin{equation}
    ...
    \end{pmatrix} = (\mu_{1}^{+}, \mu_{1}^{-}) \label{ch2-eq-mu-1-matrix}
\end{equation}
则由积分方程 \eqref{ch2-eq-integral-mu} 可得如下 Voltarra 积分方程
\begin{align}
    \mu_{1}^{+}(x,t,z) & = (1,0)^T - \int^{x}_{-\infty}  \begin{pmatrix} 1 &  \\  & e^{ \rmi z (x - y)} \end{pmatrix} P \mu_{1}^{+} \rmd y \label{ch2-eq-Voltarra-mu-1} \\
    \mu_{1}^{-}(x,t,z) & = (0,1)^T - \int^{x}_{-\infty}  \begin{pmatrix} e^{-\rmi z (x - y)} &  \\  & 1 \end{pmatrix} P \mu_{1}^{-} \rmd y \label{ch2-eq-Voltarra-mu-2}
\end{align}
...
\begin{proof}
    ...
    Step1. 解的存在性: 事实上方程 \eqref{ch2-eq-Voltarra-mu-1} 有如下 Neumann 级数分解
    \begin{equation}
        \mu_{1}^{+} = \sum_{n=0}^{\infty} c_{n}(x,z), \quad c_{n+1} = \int_{-\infty}^{x} \begin{pmatrix} 1 &  \\  & e^{ \rmi z (x - y)} \end{pmatrix} P c_{n}(y,z) \rmd y \label{ch2-eq-Neumann-series-mu-1-plus}
    \end{equation}
    其分量形式为
    \begin{equation}
    ...
    \begin{equation}
        c_{2n}^{(1)}(x,z) = \int_{-\infty}^{x} q(y) c_{2n-1}^{(2)}(y,z) \rmd y, \quad c_{2n+1}^{(2)}(x,z) = \int_{-\infty}^{x} e^{2 \rmi z(x-y)} p(y) c_{2n}^{(1)}(y,z) \rmd y \label{ch2-eq-Neumann-series-coefficient}
    \end{equation}
    对上面方程组进一步简化, 引入如下恒等式
    \begin{equation}
        \begin{aligned}
        ...
        \end{aligned} \label{ch2-eq-Neumann-series-coefficient-identity}
    \end{equation}
    利用归纳法可证明当 $ \Im(z) > 0 $ 时, 有
    \begin{equation}
        |c_{2n+1}^{(2)} | = \frac{u^{n+1}(x)}{(n+1)!} \frac{v^{n}(x)}{n!} , \quad |c_{2n}^{(1)} | = \frac{u^{n}(x)}{n!} \frac{v^{n}(x)}{n!} \label{ch2-eq-Recurrence-relation}
    \end{equation}
    其中
    \begin{equation}
        u(x) = \int_{-\infty}^{x} |p(y)| \rmd y, \quad v(x) = \int_{-\infty}^{x} |q(y)| \rmd y
    \end{equation}
    事实上, 当 $ \Im(z) > 0 $ 时, $ |e^{2 \rmi z (x-y)} | \leq 1 $, 利用 $ c_{0}^{(1)} = 1 $, 由 \eqref{ch2-eq-Recurrence-relation} 可得
    \begin{equation}
    ...
    \begin{equation}
        ...
        \end{aligned}
    \end{equation}
    不妨设对于 $ k \leq l $ 时满足 $  | c_{2l+1}^{(2)} | \leq \frac{u^{l+1}(x)}{(l+1)!} \frac{v^{l}(x)}{l!}, | c_{2l}^{(1)} | \leq \frac{u^{l}(x)}{l!} \frac{v^{l}(x)}{l!} $, 下证对于 $ k = l +1 $ 仍成立, 利用 \eqref{ch2-eq-Neumann-series-coefficient-identity} 式可得
    \begin{equation}
    ...
    \end{equation}
    从而上述 \eqref{ch2-eq-Neumann-series-coefficient} 式得证. 注意到 $ | q(x) |  = | p(x) |$, 则 $ v(x) = u(x) $, 故 \eqref{ch2-eq-Neumann-series-coefficient} 式可化简为
    \begin{equation}
    ...
    Step2. 解的唯一性: 不妨设 $ \tilde{\mu}_{1}^{+} $ 为方程 \eqref{ch2-eq-Voltarra-mu-1} 的另一个解, 不妨设 $ h = \mu_{1}^{+} - \tilde{\mu}_{1}^{+} $, 则有
    \begin{equation}
        \| h(x,t,z) \| = \left\| \int_{-\infty}^{x} \begin{pmatrix} 1 &  \\  & e^{ \rmi z (x - y)} \end{pmatrix} P h \rmd y \right\| \leq 4 \int_{-\infty}^{x} | q | \|  h \| \rmd y
    \end{equation}
    由 Grönwall 不等式可得 $ h = 0 $, 故 $ \mu_{1}^{+} $ 为方程的唯一解, 同理可得 $ \mu_{1}^{-} $ 的解析性.
\end{proof}
...
\end{equation*}
由 \eqref{ch2-eq-mu-det} 可知, $ \mu_{1}, \mu_{2} $ 可逆, 且逆矩阵为其伴随阵, 另外基于 $ \mu_{1}, \mu_{2} $ 的列向量函数的解析性, 可得 $ \mu_{1}^{-1} $ 的第一二行在 $ \mathbb{C}_{-}, \mathbb{C}_{+} $ 上解析, $ \mu_{2}^{-1} $ 的第一二行在 $ \mathbb{C}_{+}, \mathbb{C}_{-} $ 上解析. 记为
\begin{equation}
    \mu_{1}^{-1} =
    \begin{pmatrix}
    ...
    \end{pmatrix} = \begin{pmatrix}
        - \hat{\mu_{2}^{+}} \\ \hat{\mu_{2}^{-}}
    \end{pmatrix} \label{ch2-eq-mu-1-2-inverse-matrix}
\end{equation}
利用 \eqref{ch2-eq-eig-mu-relationship}, \eqref{ch2-eq-mu-1-matrix}, \eqref{ch2-eq-mu-1-2-inverse-matrix} 可得谱函数 $ S(z) $ 的解析性
\begin{equation}
...
\end{theorem}
\begin{proof}
    由 \eqref{ch2-eq-Focus-NLS-Lax-x-Asymptotic} 有
    \begin{equation}
        \mu_{j,x}(x,t,z) = \rmi z [\sigma_{3}, \mu_{j}(x,t,z)] = P \mu_{j}(x,t,z) \label{ch2-eq-mu-diff-x}
    \end{equation}
    ...
    \end{equation}
    将 \eqref{ch2-eq-mu-diff-x} 带入上式有
    \begin{equation}
    ...
    \end{equation}
    由 \eqref{ch2-eq-Voltarra-mu-1}, \eqref{ch2-eq-Neumann-series-mu-1-plus} 可得 $ \mu_{j}^{\dagger}, \mu_{j}^{-1} $ 满足相同的一次线性微分方程, 具有相同的渐进性, 又因为
    \begin{equation}
        \mu_{j}^{\dagger}(x,t,z^{*}) , \mu_{j}^{-1}(x,t,z) \to I , \quad | x | \to \infty \label{ch2-eq-mu-Hermitian-asymptotic}
    \end{equation}
    因此两者相等, 得到对称关系 $ \mu_{j}^{\dagger}(x,t,z^{*}) = \mu_{j}^{-1}(x,t,z), (j = 1,2) $. 接下来考虑 $ S $ 的对称性, 将 \eqref{ch2-eq-eig-mu-relationship} 式改写为
    \begin{equation}
        S(z) = e^{\rmi \theta(z) \hat{\sigma}_{3}} (\mu_{2}^{-1}\mu_{1}) = \hat{E}(\mu_{2}^{-1}\mu_{1})
    \end{equation}
    由 \eqref{ch2-eq-mu-Hermitian-asymptotic} 可得如下
    \begin{equation}
    ...
    比较对应元素, 有
    \begin{equation}
        s_{11}^{*}(z^{*}) = s_{22}(z), s_{12}^{*}(z^{*}) = - s_{21}(z) \label{ch2-eq-S-symmetry}
    \end{equation}
\end{proof}

\section{相关的 RH 问题}
\subsection{规范 RH 问题}
基于第 \ref{ch2-sec-analytic-symmetry} 节的结论, 我们构造 RH 问题, 引入记号
\begin{equation}
    H_{1} = \begin{pmatrix}
        1 & \\ & 0
    \end{pmatrix}, \quad
...
\begin{theorem}
    ...
    \begin{equation}
        P_{+}^{\dagger}(x,t,z^{*}) =  P_{-}(x,t,z) \label{ch2-eq-P-symmetry}
    \end{equation}
\end{theorem}
\begin{proof}
    利用 \eqref{ch2-eq-mu-Hermitian-asymptotic} 可得
    \begin{equation}
        \begin{aligned}
...
\begin{definition}
    ...
    \begin{align}
        P_{\pm}(x,t,z) \in C(\mathbb{C}_{\pm})  \label{ch2-eq-RHP-1}      \\
        P_{-}P_{+} = G(x,t,z) \quad z \in \mathbb{R} \label{ch2-eq-RHP-2} \\
        P_{\pm} \to I \quad z \to \infty \label{ch2-eq-RHP-3}
    \end{align}
    其中跳跃矩阵为
    \begin{equation}
    ...
    \begin{equation}
        \begin{aligned}
            G & = P_{-}P_{+} = (H_{1} \mu_{1}^{-1} + H_{2} \mu_{2}^{-1})(\mu_{1} H_{1} + \mu_{2} H_{2})                                                                     \\
              & \overset{\eqref{ch2-eq-eig-mu-relationship}}{=} [H_{1}(\hat{E}^{-1} S^{-1})\mu_{2}^{-1} + H_{2}\mu_{2}^{-1}]  [\mu_{2}(\hat{E}^{-1} S)H_{1} + \mu_{2}H_{2}] \\
...
下面分两种情况讨论 RH 问题 \eqref{ch2-eq-RHP-1} - \eqref{ch2-eq-RHP-3} 的解

Case1. 如果
\begin{equation}
    \det P_{\pm} (z) \neq 0 (\forall z \in \mathbb{C}), \label{ch2-eq-RHP-regular}
\end{equation}
则称 RH 问题 \eqref{ch2-eq-RHP-1} - \eqref{ch2-eq-RHP-3} 为正则的, 将方程 \eqref{ch2-eq-RHP-2} 改写为
\begin{equation}
...
Case2. 如果条件 \eqref{ch2-eq-RHP-regular} 不满足, 则称 RH 问题为非正则的, 假设 $ \det P_{\pm} $ 在某些离散的点处为零, 由谱函数对称性 \eqref{ch2-eq-S-symmetry} 有
\begin{equation}
...
\begin{equation}
    P_{+}(z_{j}) w_{j}(z_{j}) = 0, \quad w_{j}^{*}(z_{j}^{*}) P_{-}(z_{j}^{*})  = 0 \label{ch2-eq-RHP-singular}
\end{equation}
对上式取共轭转置, 则有 $ w_{j}^{\dagger}(z_{j}) P_{+}^{\dagger}(z_{j})  = 0 $, 再利用对称性 \eqref{ch2-eq-P-symmetry} 有
\begin{equation}
    w_{j}^{\dagger}(z_{j}) P_{-}(z_{j}^{*})  = 0 \label{ch2-eq-RHP-singular-Hermitian}
\end{equation}
比较可得 $ w_{j}^{\dagger}(z) = w_{j}^{*}(z^{*}) $.

非正则 RH 问题 \eqref{ch2-eq-RHP-1} - \eqref{ch2-eq-RHP-3} 的解可由下面定理给出
\begin{theorem}{Zakharov, Shabat, 1979}
    带有零点结构 \eqref{ch2-eq-RHP-singular} 的非正则 RH 问题 \eqref{ch2-eq-RHP-1} - \eqref{ch2-eq-RHP-3} 可分解为
    \begin{equation}
        P_{+}(z) = \hat{P_{+}}(z) \Gamma(z) , \quad P_{-}(z) = \Gamma^{-1}(z) \hat{P_{-}}(z) \label{ch2-eq-RHP-decomposition}
    \end{equation}
    其中
    \begin{equation}
        \Gamma(z) = I + \sum_{k,j = 1}^{N} \frac{w_{k}(M^{-1})_{kj} w_{j}^{*}}{z - z_{j}^{*}}, \quad \Gamma(z)^{-} = I - \sum_{k,j = 1}^{N} \frac{w_{k}(M^{-1})_{kj} w_{j}^{*}}{z - z_{k}} \label{ch2-eq-Gamma}
    \end{equation}
...
\end{theorem}
\begin{proof}
    非正则 RH 问题  \eqref{ch2-eq-RHP-1} - \eqref{ch2-eq-RHP-3} 是由于 $ 2N $ 个离散谱上的 $ \det P_{+}(z_{j}) = \det P_{-}(Z_{j^{*}}) = 0 (j = 1,2, \dots N) $ 造成的. 所以主要任务是消除这些零点结构, 并且去除 $ P_{+}(z_{j}), P_{-}(z_{j}^{*}) $ 在 $ z_{j}, z_{j}^{*} $ 上的零点结构, 为此需定义单极点矩阵
    \begin{equation}
    ...
    \end{equation}
    定义矩阵函数 $ R_{1}^{+}(z) = P_{+}(z) \Gamma_{+}^{-1}(z), R_{1}^{-1}= \Gamma_{1}(z) P_{-}(z) $, 由 \eqref{ch2-eq-RHP-singular} - \eqref{ch2-eq-RHP-singular-Hermitian} 知
    \begin{equation}
        \begin{aligned}
    ...
    \end{equation}
    与 \eqref{ch2-eq-RHP-singular} 比较可得
    \begin{equation}
    ...
    其中
    \begin{equation}
        \Gamma_{j}(z) = I + \frac{z_{j}^{*} - z_{j}}{z - z_{j}^{*}} \cdot \frac{v_{j}v_{j}^{*}}{v_{j}^{*}v_{j}}, \quad \Gamma_{j}^{-1}(z) = I - \frac{z_{j}^{*} - z_{j}}{z - z_{j}} \cdot \frac{v_{j}v_{j}^{*}}{v_{j}^{*}v_{j}} \label{ch2-eq-Gamma-j}
    \end{equation}
    ...
    \begin{equation}
        \begin{aligned}
            \Gamma_{z} = \Gamma_{N}(z) \cdots \Gamma_{1}(z), & \quad \Gamma_{z}^{-1} = \Gamma_{1}^{-1}(z) \cdots \Gamma_{N}^{-1}(z) \label{ch2-eq-Gamma-composition} \\
    ...
    \end{equation}
    得到分解 \eqref{ch2-eq-RHP-decomposition}. 下证 \eqref{ch2-eq-Gamma}. 注意到 $ \Gamma(z), \Gamma^{-1}(z) $ 分别为具有单极点 $ z_{j}, z_{j}^{*} (1 \leq j \leq N)$ 的亚纯函数, 以及分解 \eqref{ch2-eq-Gamma-j}, \eqref{ch2-eq-Gamma-composition}, 寻找的向量使得
    \begin{equation}
        \Gamma(z) = I + \sum_{j=1}^{N} \frac{\xi_{j}w_{j}^{*}}{z-z_{j}^{*}}, \quad \Gamma^{-1}(z) = I - \sum_{j=1}^{N} \frac{w_{j}^{*}\xi_{j}}{z - z_{j}} \label{ch2-eq-Gamma-expRession}
    \end{equation}
    注意到 $ \Gamma(z) \Gamma^{-1}(z) = I $ 对任意 $ z $ 都成立, 当然也在 $ z = z_{k} $ 处成立, 即 $ \Gamma(z) \Gamma^{-1}(z) = I $ 在 $ z = z_{k} $ 处正则, 为保证 $ \Gamma(z) \Gamma^{-1}(z) = I $ 在 $ z = z_{k} $ 处成立, 只需让其留数为 $ 0 $, 因此利用 \eqref{ch2-eq-Gamma-expRession} 可得
    \begin{equation}
        \begin{aligned}
    ...
    \begin{equation}
        ...
        \end{pmatrix}
    \end{equation}
    可得 $ \xi_{j} = \sum_{k =1}^{N}(M^{-1})_{kj} w_{k} $. 最后将其带入 \eqref{ch2-eq-Gamma-expRession}得到 \eqref{ch2-eq-Gamma}
\end{proof}

\section{NLS 方程的 N 孤子解}
\subsection{矩阵向量解的时空演化}
对方程 \eqref{ch2-eq-RHP-singular} 第一个式子两边分别对 $ x, t $ 求导, 可得
\begin{equation}
    P_{+, x} w_{j} + P_{+} w_{j, x} = 0, \quad P_{+, t} w_{j} + P_{+} w_{j, t} = 0 \label{ch2-eq-Focus-NLS-Lax-x-t-Asymptotic}
\end{equation}
利用 $ P_{+} $ 的定义与 Lax 对\eqref{ch2-eq-Focus-NLS-Lax-x-Asymptotic}, \eqref{ch2-eq-Focus-NLS-Lax-t-Asymptotic} 可得
\begin{equation}
    \begin{aligned}
    ...
    \end{aligned} \label{ch2-eq-P-x}
\end{equation}
同理有
\begin{equation}
    P_{+,t} = - \rmi z_{j}^{2} [\sigma_{3}, P_{+}] + Q P_{+} \label{ch2-eq-P-t}
\end{equation} 将 \eqref{ch2-eq-P-x}, \eqref{ch2-eq-P-t} 代入 \eqref{ch2-eq-Focus-NLS-Lax-x-t-Asymptotic} 且由于 $ P_{+}w_{j} = 0, w_{j}P = 0 $, 有
\begin{equation}
...
再由
\begin{equation}
    \Gamma(z) = I + \sum_{k,j = 1}^{N} \frac{w_{k}(M^{-1})_{kj}w_{j}^{*}}{z-z_{j}^{*}} \implies \Gamma(z) = I + \frac{1}{z} \sum_{k,j = 1}^{N} w_{k}(M^{-1})_{kj}w_{j}^{*} + O(z^{-2}) \label{ch2-eq-Gamma-Asymptotic}
\end{equation}
将上渐进式带入 \eqref{ch2-eq-RHP-decomposition}, 比较 $ z^{-1} $ 的次数可得
\begin{equation}
    P_{+}^{(1)} = \frac{1}{2 \pi \rmi} \int_{-\infty}^{+\infty} \Gamma(z) \hat{G} \Gamma^{-1}(z) \hat{P_{+}^{-1}} \rmd s = \sum_{k,j = 1}^{N} w_{k} (M^{-1})_{kj} w_{j}^{*} \label{ch2-eq-P-plus-1}
\end{equation}
特别的, 当散射数据 $ S_{12} = S_{21} = 0 $ 时, 有 $ \hat{G} = I - G = 0 $. 故上式\eqref{ch2-eq-P-plus-1} 可简化为
\begin{equation}
...
\end{equation}
再由 \eqref{ch2-eq-Gamma-Asymptotic}
\begin{equation}
    \begin{aligned}
        q(x,t) & = 2 \rmi \lim_{z \to \infty} (zP_{+})_{12} = 2 \rmi (P_{+}^{(1)})_{12}                                                                                             \\
               & = 2 \rmi \sum_{k,j}^{N}(w_{k}w_{j}^{*})_{12}(M^{-1})_{kj} = 2 \rmi \sum_{k,j}^{N} c_{k}e^{\lambda_{k} - \lambda_{j}^{*}}(M^{-1})_{kj} \label{ch2-eq-NLS-N-soliton}
    \end{aligned}
\end{equation}
令
\begin{equation}
...
\begin{equation}
    \begin{aligned}
        ...
        %\overset{\mathclap{\text{这是较长的文本，需要等号自适应}}}{=} \\
               ...
               & = -\det M \sum c_{k}e^{\lambda_{k} - \lambda_{j}^{*}} \qquad \Leftarrow \left( (M^{-1})_{jk} (M^{*}) = \det(M) M^{-1} \right)
    \end{aligned}
\end{equation}
因此
\begin{equation}
    \sum_{k=1}^{N} c_{k} e^{\lambda_{k} - \lambda_{j}^{*}}(M^{-1})_{jk} = - \frac{\det R}{\det M} \label{ch2-eq-q}
\end{equation}
将 \eqref{ch2-eq-q} 带入 \eqref{ch2-eq-NLS-N-soliton} 可得 NLS 方程的 N 孤子解
\begin{equation}
