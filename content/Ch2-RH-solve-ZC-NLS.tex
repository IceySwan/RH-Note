...
\begin{proof}
    ...
    \begin{equation}
        \mu_{1}^{+} = \sum_{n=0}^{\infty} c_{n}(x,z), \quad c_{n+1} = \int_{-\infty}^{x} \begin{pmatrix} 1 &  \\  & e^{ \rmi z (x - y)} \end{pmatrix} P c_{n}(y,z) \rmd y \label{ch2-eq:Neumann-series-mu-1+}
    \end{equation}
    其分量形式为
    \begin{equation}
...
\begin{proof}
    ...
    \end{equation}
    由 \eqref{ch2-eq:Voltarra-mu-1}, \eqref{ch2-eq:Neumann-series-mu-1+} 可得 $ \mu_{j}^{\dagger}, \mu_{j}^{-1} $ 满足相同的一次线性微分方程, 具有相同的渐进性, 又因为
    \begin{equation}
...
\section{相关的 RH 问题}
\subsection{规范 RH 问题}
基于 \eqref{ch2-sec:analytic-symmetry} 节的结论, 我们构造 RH 问题, 引入记号
\begin{equation}
    H_{1} = \begin{pmatrix}
        1 & \\ & 0
    \end{pmatrix}, \quad
...
\begin{equation}
    P_{+}^{(1)} = \frac{1}{2 \pi \rmi} \int_{-\infty}^{+\infty} \Gamma(z) \hat{G} \Gamma^{-1}(z) \hat{P_{+}^{-1}} \rmd s = \sum_{k,j = 1}^{N} w_{k} (M^{-1})_{kj} w_{j}^{*} \label{ch2-eq:P-+-1}
\end{equation}
特别的, 当散射数据 $ S_{12} = S_{21} = 0 $ 时, 有 $ \hat{G} = I - G = 0 $. 故上式\eqref{ch2-eq:P-+-1} 可简化为
\begin{equation}
