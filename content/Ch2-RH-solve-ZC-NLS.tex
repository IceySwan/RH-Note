...
%\end{equation}
NLS 方程 (\ref{ch2-eq:Focus-NLS}) 具有如下矩阵形式的 Lax 对 
\begin{align}
...
由于 $ q_{0}(x) \in \mathcal{S}(\mathbb{R}) $, 且 $ q(x,t), q_{x}(x,t) \to 0, (x \to \infty) $, 故 x 足够大时, $ P, Q $ 可忽略, 从而 Lax 对 (\ref{ch2-eq:Focus-NLS-Lax-x}) - (\ref{ch2-eq:Focus-NLS-Lax-t}) 可近似为
\begin{equation*}
...
\end{align}
带入 Lax 对 (\ref{ch2-eq:Focus-NLS-Lax-x}) - (\ref{ch2-eq:Focus-NLS-Lax-t}) 可得
\begin{align}
...
\end{equation}
由 (\ref{ch2-eq:Focus-NLS-Lax-x-Asymptotic}) 式可得
\begin{align}
...
\begin{proof}
  ...
  \end{equation}
  其中 $ \mu^{(0)}, \mu^{(1)} $ 与 $ z $ 无关, 将上式代入 (\ref{ch2-eq:Focus-NLS-Lax-x-Asymptotic}), (\ref{ch2-eq:Focus-NLS-Lax-t-Asymptotic}) 比较 $ z $ 的次数可得
  \begin{equation}
    \begin{aligned}
  ...
  \begin{equation}
    ...
    \implies \mu^{(0)}_x = 0
  \end{equation}
  故 $ \mu^{(0)} $ 为与 $ x $ 无关的对角矩阵, 因此 (\ref{ch2-eq:mu-Taylor}) 式对 $ x, z $ 同时取极限, 并交换极限顺序可得 
  \begin{equation}
    \lim_{z \to \infty} \lim_{x \to \infty} \mu = \lim_{x \to \infty} \lim_{z \to \infty} \left( \mu^{(0)} + O(z^{(-1)}) \right)
  \end{equation}
  利用 (\ref{ch2-eq:mu-asymptomatic}), (\ref{ch2-eq:mu-Taylor}) 式可得 $ \mu^{(0)} = I $, 故 $ \mu \to I (z \to \infty) $.
\end{proof}
再将 $ \mu^{(0)} = I $ 带入 (\ref{ch2-eq:mu-Taylor-t-z}) 式比较矩阵对角元素得
\begin{equation}
...
由于 (\ref{ch2-eq:diffision-mu}) 为全微分形式, 积分与路径无关, 故选择两个特殊路径: 
\begin{equation*}
  (- \infty, t) \to(x,t) , \quad (+ \infty, t) \to(x,t)
\end{equation*}
因此可获得 Lax 对 (\ref{ch2-eq:Focus-NLS-Lax-x-Asymptotic}), (\ref{ch2-eq:Focus-NLS-Lax-t-Asymptotic}) 的两个特征函数 
\begin{equation}
...
\end{equation}
\begin{proof}
  由 (\ref{ch2-eq:diffision-mu}) 式, 由 $ (\hat{E} \mu)_{x} = \hat{E} P \mu $ 可得
  \begin{subequations}
  \begin{align}
...
\end{equation}
故对 (\ref{ch2-eq:eig-mu-relationship}) 两边取行列式有 $ \det S(z) = 1 $. 
...
\end{equation}
则由积分方程 (\ref{ch2-eq:integral-mu}) 可得如下 Voltarra 积分方程
  \begin{align}
...
\begin{proof}
  ...
  Step1. 解的存在性: 事实上方程 (\ref{ch2-eq:Voltarra-mu-1}) 有如下 Neumann 级数分解
  \begin{equation}
  ...
  \end{equation}
  事实上, 当 $ \Im(z) > 0 $ 时, $ |e^{2 \rmi z (x-y)} | \leq 1 $, 利用 $ c_{0}^{(1)} = 1 $, 由 (\ref{ch2-eq:Recurrence-relation}) 可得
  \begin{equation}
  ...
  \begin{equation}
    ...
    \end{aligned}
  \end{equation}
  不妨设对于 $ k \leq l $ 时满足 $  | c_{2l+1}^{(2)} | \leq \frac{u^{l+1}(x)}{(l+1)!} \frac{v^{l}(x)}{l!}, | c_{2l}^{(1)} | \leq \frac{u^{l}(x)}{l!} \frac{v^{l}(x)}{l!} $, 下证对于 $ k = l +1 $ 仍成立, 利用 (\ref{ch2-eq:Neumann-series-coefficient-identity}) 式可得
    \begin{equation}
    ...
    \end{equation}
  从而上述 (\ref{ch2-eq:Neumann-series-coefficient}) 式得证. 注意到 $ | q(x) |  = | p(x) |$, 则 $ v(x) = u(x) $, 故 (\ref{ch2-eq:Neumann-series-coefficient}) 式可化简为
  \begin{equation}
  ...
  Step2. 解的唯一性: 不妨设 $ \tilde{\mu}_{1}^{+} $ 为方程 (\ref{ch2-eq:Voltarra-mu-1}) 的另一个解, 不妨设 $ h = \mu_{1}^{+} - \tilde{\mu}_{1}^{+} $, 则有 
  \begin{equation}
...
\end{equation*}
由 (\ref{ch2-eq:mu-det}) 可知, $ \mu_{1}, \mu_{2} $ 可逆, 且逆矩阵为其伴随阵, 另外基于 $ \mu_{1}, \mu_{2} $ 的列向量函数的解析性, 可得 $ \mu_{1}^{-1} $ 的第一二行在 $ \mathbb{C}_{-}, \mathbb{C}_{+} $ 上解析, $ \mu_{2}^{-1} $ 的第一二行在 $ \mathbb{C}_{+}, \mathbb{C}_{-} $ 上解析. 记为
\begin{equation}
  \mu_{1}^{-1} = 
  \begin{pmatrix} 
...
\end{equation}
利用 (\ref{ch2-eq:eig-mu-relationship}), (\ref{ch2-eq:mu-1-matrix}), (\ref{ch2-eq:mu-1-2-inverse-matrix}) 可得谱函数 $ S(z) $ 的解析性
\begin{equation}
...
\end{theorem}
\begin{proof}
  由 (\ref{ch2-eq:Focus-NLS-Lax-x-Asymptotic}) 有
  \begin{equation}
  ...
  \end{equation}
  由 (\ref{ch2-eq:Voltarra-mu-1}), (\ref{ch2-eq:Neumann-series-mu-1+}) 可得 $ \mu_{j}^{\dagger}, \mu_{j}^{-1} $ 满足相同的一次线性微分方程, 具有相同的渐进性, 又因为
  \begin{equation}
    \mu_{j}^{\dagger}(x,t,z^{*}) , \mu_{j}^{-1}(x,t,z) \to I , \quad | x | \to \infty \label{ch2-eq:mu-Hermitian-asymptotic}
  \end{equation}
  因此两者相等, 得到对称关系 $ \mu_{j}^{\dagger}(x,t,z^{*}) = \mu_{j}^{-1}(x,t,z), (j = 1,2) $. 接下来考虑 $ S $ 的对称性, 将 (\ref{ch2-eq:eig-mu-relationship}) 式改写为
  \begin{equation}
    S(z) = e^{\rmi \theta(z) \hat{\sigma}_{3}} (\mu_{2}^{-1}\mu_{1}) = \hat{E}(\mu_{2}^{-1}\mu_{1}) 
  \end{equation}
  由 (\ref{ch2-eq:mu-Hermitian-asymptotic}) 可得如下
  \begin{equation}
...
\section{相关的 RH 问题}
\subsection{规范 RH 问题}
基于 \ref{ch2-sec:analytic-symmetry} 节的结论, 我们构造 RH 问题, 引入记号
\begin{equation}
  H_{1} = \begin{pmatrix}
    1 & \\ & 0
  \end{pmatrix}, \quad 
...
\end{theorem}
\begin{proof}
  利用 (\ref{ch2-eq:mu-Hermitian-asymptotic}) 可得
  \begin{equation}
    \begin{aligned}
  ...
  \begin{equation}
    \begin{aligned}
      G &= P_{-}P_{+} = (H_{1} \mu_{1}^{-1} + H_{2} \mu_{2}^{-1})(\mu_{1} H_{1} + \mu_{2} H_{2}) \\
        &\overset{(\ref{ch2-eq:eig-mu-relationship})}{=} [H_{1}(\hat{E}^{-1} S^{-1})\mu_{2}^{-1} + H_{2}\mu_{2}^{-1}]  [\mu_{2}(\hat{E}^{-1} S)H_{1} + \mu_{2}H_{2}] \\
...
下面分两种情况讨论 RH 问题 (\ref{ch2-eq:RHP-1}) - (\ref{ch2-eq:RHP-3}) 的解

Case1. 如果 
\begin{equation}
  \det P_{\pm} (z) \neq 0 (\forall z \in \mathbb{C}), \label{ch2-eq:RHP-regular}
\end{equation} 
则称 RH 问题 (\ref{ch2-eq:RHP-1}) - (\ref{ch2-eq:RHP-3}) 为正则的, 将方程 (\ref{ch2-eq:RHP-2}) 改写为
\begin{equation}
...
Case2. 如果条件 (\ref{ch2-eq:RHP-regular}) 不满足, 则称 RH 问题为非正则的, 假设 $ \det P_{\pm} $ 在某些离散的点处为零, 由谱函数对称性 (\ref{ch2-eq:S-symmetry}) 有
\begin{equation}
...
\end{equation}
对上式取共轭转置, 则有 $ w_{j}^{\dagger}(z_{j}) P_{+}^{\dagger}(z_{j})  = 0 $, 再利用对称性 (\ref{ch2-eq:P-symmetry}) 有
\begin{equation}
  w_{j}^{\dagger}(z_{j}) P_{-}(z_{j}^{*})  = 0 \label{ch2-eq:RHP-singular-Hermitian}
\end{equation}
比较可得 $ w_{j}^{\dagger}(z) = w_{j}^{*}(z^{*}) $.

非正则 RH 问题 (\ref{ch2-eq:RHP-1}) - (\ref{ch2-eq:RHP-3}) 的解可由下面定理给出
\begin{theorem}{Zakharov, Shabat, 1979}
  带有零点结构 (\ref{ch2-eq:RHP-singular}) 的非正则 RH 问题 (\ref{ch2-eq:RHP-1}) - (\ref{ch2-eq:RHP-3}) 可分解为
  \begin{equation}
...
\end{theorem}
\begin{proof}
  非正则 RH 问题  (\ref{ch2-eq:RHP-1}) - (\ref{ch2-eq:RHP-3}) 是由于 $ 2N $ 个离散谱上的 $ \det P_{+}(z_{j}) = \det P_{-}(Z_{j^{*}}) = 0 (j = 1,2, \dots N) $ 造成的. 所以主要任务是消除这些零点结构, 并且去除 $ P_{+}(z_{j}), P_{-}(z_{j}^{*}) $ 在 $ z_{j}, z_{j}^{*} $ 上的零点结构, 为此需定义单极点矩阵
  \begin{equation}
  ...
  \end{equation}
  定义矩阵函数 $ R_{1}^{+}(z) = P_{+}(z) \Gamma_{+}^{-1}(z), R_{1}^{-1}= \Gamma_{1}(z) P_{-}(z) $, 由 (\ref{ch2-eq:RHP-singular}) - (\ref{ch2-eq:RHP-singular-Hermitian}) 知
  \begin{equation}
    \begin{aligned}
  ...
  \end{equation}
  与 (\ref{ch2-eq:RHP-singular}) 比较可得
  \begin{equation}
  ...
  \end{equation}
  得到分解 (\ref{ch2-eq:RHP-decomposition}). 下证 (\ref{ch2-eq:Gamma}). 注意到 $ \Gamma(z), \Gamma^{-1}(z) $ 分别为具有单极点 $ z_{j}, z_{j}^{*} (1 \leq j \leq N)$ 的亚纯函数, 以及分解 (\ref{ch2-eq:Gamma-j}), (\ref{ch2-eq:Gamma-composition}), 寻找的向量使得
  \begin{equation}
  ...
  \end{equation} 
  注意到 $ \Gamma(z) \Gamma^{-1}(z) = I $ 对任意 $ z $ 都成立, 当然也在 $ z = z_{k} $ 处成立, 即 $ \Gamma(z) \Gamma^{-1}(z) = I $ 在 $ z = z_{k} $ 处正则, 为保证 $ \Gamma(z) \Gamma^{-1}(z) = I $ 在 $ z = z_{k} $ 处成立, 只需让其留数为 $ 0 $, 因此利用 (\ref{ch2-eq:Gamma-expRession}) 可得
  \begin{equation}
    \begin{aligned}
  ...
  \begin{equation}
    ...
    \end{pmatrix}
  \end{equation}
  可得 $ \xi_{j} = \sum_{k =1}^{N}(M^{-1})_{kj} w_{k} $. 最后将其带入 (\ref{ch2-eq:Gamma-expRession})得到 (\ref{ch2-eq:Gamma})
\end{proof}

\section{NLS 方程的 N 孤子解}
\subsection{矩阵向量解的时空演化}
对方程 (\ref{ch2-eq:RHP-singular}) 第一个式子两边分别对 $ x, t $ 求导, 可得
\begin{equation}
  P_{+, x} w_{j} + P_{+} w_{j, x} = 0, \quad P_{+, t} w_{j} + P_{+} w_{j, t} = 0 \label{ch2-eq:Focus-NLS-Lax-x-t-Asymptotic}
\end{equation}
利用 $ P_{+} $ 的定义与 Lax 对(\ref{ch2-eq:Focus-NLS-Lax-x-Asymptotic}), (\ref{ch2-eq:Focus-NLS-Lax-t-Asymptotic}) 可得
\begin{equation}
  \begin{aligned}
...
\end{equation} 将 (\ref{ch2-eq:P-x}), (\ref{ch2-eq:P-t}) 代入 (\ref{ch2-eq:Focus-NLS-Lax-x-t-Asymptotic}) 且由于 $ P_{+}w_{j} = 0, w_{j}P = 0 $, 有
\begin{equation}
...
\end{equation}
将上渐进式带入 (\ref{ch2-eq:RHP-decomposition}), 比较 $ z^{-1} $ 的次数可得
\begin{equation}
...
\end{equation}
特别的, 当散射数据 $ S_{12} = S_{21} = 0 $ 时, 有 $ \hat{G} = I - G = 0 $. 故上式(\ref{ch2-eq:P-+-1}) 可简化为
\begin{equation}
...
\end{equation}
再由 (\ref{ch2-eq:Gamma-Asymptotic}) 
\begin{equation}
  \begin{aligned}
...
\end{equation}
将 (\ref{ch2-eq:q}) 带入 (\ref{ch2-eq:NLS-N-soliton}) 可得 NLS 方程的 N 孤子解
\begin{equation}
