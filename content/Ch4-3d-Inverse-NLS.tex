...
\end{align}
其中若未声明变量默认为 $(x,t)$, 下同. 考虑相容性条件. 我们有 $ p_{1} $ 只可能为 $ p^{*}(x,t), p^{*}(-x,t), p(x,-t), p(-x,-t) $, $ q_1 $ 同理. 则得到如下四种约化
\begin{align}
    ...
    r_{1} & = a p(-x,-t) + b q(-x,-t) , \quad r_{2} = b p(-x,-t) + d q(-x,-t), \label{ch4-eq-reduction-reverse-x-t}
\end{align}
\begin{remark}
...
\begin{equation}
    (31) = a (13)^{*} + b (23)^{*} \quad (32) = b^{*} (13)^{*} + d (23)^{*}
\end{equation}
方程约化为
\begin{align}
...
此时 $r_1 = ap^{*}(-x) + bp(-t), r_2 = b^{*}p^{*}(-x) + ap(-t) $, 方程约化为
\begin{equation}
...
此时 $ a = d $, 则 $ r_1 = ap(-x,-t) + bp(-t), r_2 = bp(-x,-t) + dp(-t) $, 此时有 $ (23) = (13)(-x) $. 方程约化为
\begin{equation}
    ip_t + p_{xx} + 2p \left( app(-x,-t) + bpp(-t) + bp(-x,-t)p(-x) + ap(-t)p(-x) \right) = 0
\end{equation}

\subsubsection*{Case 4.2 继续增加约化条件 $ q = p^{*}(-t) $}
此时 $ a^{*} = d , b \in \mathbb{R}$, 则 $ r_1 = ap(-x,-t) + bp^{*}(-t), r_2 = bp(-x,-t) + dp^{*}(-t)$, 方程约化为
\begin{equation}
    ip_t + p_{xx} + 2p \left[ app(-x,-t) + bpp^{*}(-t) + bp(-x,-t)p^{*}(-t) + a^{*}p^{*}(-t)p^{*}(-x) \right] = 0
\end{equation}
...
注意到
\begin{equation}
    E^{*}(\zeta^{*}) = E^{\dagger}(\zeta^{*}) = E^{-1}(\zeta), \quad E^{*}(-x, t-\zeta^{*}) = E^{\dagger}(-x, t-\zeta^{*}) = E^{-1}(x,t\zeta)
\end{equation}
...
\begin{equation}
    \begin{aligned}
        \hat{E}^{-1} J_{x} + \rmi \zeta \hat{E}^{-1} [\Lambda, J]   & = \hat{E}^{-1} Q J, \\
        \hat{E}^{-1} J_{t} - 2 \rmi \zeta \hat{E}^{-1} [\Lambda, J] & = \hat{E}^{-1} R J.
    \end{aligned}
\end{equation}
其中 $\hat{E}^{-1} X = E^{-1} X E$, 则易得全微分形式
\begin{equation}
    \rmd(\hat{E}^{-1} J) = \hat{E}^{-1} [(Q \rmd x + R \rmd t) J]. \label{ch4-eq-reverse-x-volterra}
\end{equation}
\subsection{解析性}
...
\begin{proof}
    ...
    \begin{equation}
        \begin{aligned}
            \hat{E}^{-1} J_{-}(x, t, \zeta)|_{-\infty}^{x} & = \int_{-\infty}^{x} \hat{E}^{-1}(y,t) Q(y, t) J_{-}(y, t, \zeta) \rmd y                                              \\
    ...
    \end{equation}
    若设 $J_{-} = ([J_{-}]_{1}, [J_{-}]_{2}, [J_{-}]_{3})$, 现考虑 $[J_{-}]_{1}$, 由上式得
    \begin{equation}
        \begin{aligned}
            \relax  [J_{-}]_{1}(x, t, \zeta) & = (1,0,0)^{T} + \int_{-\infty}^{x} e^{\rmi \zeta (y-x) \Lambda} Q(y, t) J_{-}(y, t, \zeta) e^{\rmi \zeta (y-x)}(1,0,0)^{T} \rmd y \\
    ...
    定义 Neumann 级数及递推序列
    \begin{equation}
        [J_{-}]_{1}(x,t,\zeta) = \sum_{n=0}^{\infty}c_{n},      \quad
    ...
    易见 $\|c_{0}\| \leq \omega^{0}/0!, \| c_{1} \| \leq \omega^{1}/1!$, 由数学归纳法, 不妨设 $\| c_{n} \| \leq \omega^{n}/n!$, 则
    \begin{equation}
...
\begin{equation}
    P^{+} = [[\phi_{+}]_{1}, [\phi_{+}]_{2}, [\phi_{-}]_{3}] E^{-1} = J_{+}H_{2} + J_{-}H_{1}, \quad \zeta \in \mathbb{C}_{+} \label{ch4-eq-RHP-P1}
\end{equation}
...
\begin{equation}
    \phi_{\pm}^{-1} = ([\hat{\phi}_{\pm}]_{1}, [\hat{\phi}_{\pm}]_{2}, [\hat{\phi}_{\pm}]_{3})^{T},
\end{equation}
重复上述步骤有
\begin{equation}
    [\hat{\phi}_{+}]_{1}, [\hat{\phi}_{+}]_{2}, [\hat{\phi}_{-}]_{3} \in \mathbb{C}_{-}, \quad [\hat{\phi}_{-}]_{1}, [\hat{\phi}_{-}]_{2}, [\hat{\phi}_{+}]_{3} \in \mathbb{C}_{+}
\end{equation}
可得
\begin{equation}
    P^{-} = E ([\hat{\phi}_{+}]_{1}, [\hat{\phi}_{+}]_{2}, [\hat{\phi}_{-}]_{3})^{T} = H_{2}J_{+}^{-1} + H_{1} J_{-}^{-1}, \quad \zeta \in \mathbb{C}_{-} \label{ch4-eq-RHP-P-minus}
\end{equation}
...
\begin{proof}
    ...
    \begin{equation}
        \begin{aligned}
            ...
            \implies J_{x}^{\dagger}(\zeta^{*}) M_{0} = -\rmi \zeta [\Lambda, J^{\dagger}(\zeta^{*}) M_{0}] - J^{\dagger}(\zeta^{*}) M_{0} Q,
        \end{aligned}
    \end{equation}
    由 Volterra 积分的唯一性, 与伴随问题 \eqref{ch4-eq-Lax-adjoint} 对比可得 $J^{\dagger}(\zeta^{*}) M_{0} = C J^{-1}(\zeta)$, 其中 $C$ 为常数矩阵, 由边界条件可得 $x \to \pm \infty$ 时
    \begin{equation}
        J_{\pm}^{\dagger}(\zeta^{*}) M_{0} \to M_{0}, \quad J_{\pm}^{-1}(\zeta) \to C \implies C = M_{0}.
    \end{equation}
    利用 $J_{-} E = J_{+} E S(\zeta)$ 及 $E(\zeta^{*})^{\dagger} = E^{-1}$ 可得
    \begin{equation}
        S^{\dagger}(\zeta^{*}) = \left( E^{-1}(\zeta^{*}) J_{+}^{-1}(\zeta^{*}) J_{-}(\zeta^{*}) E(\zeta^{*}) \right)^{\dagger} = E^{-1} M_{0} J_{-}^{-1} J_{+} M_{0}^{-1} E = M_{0} S^{-1}(\zeta) M_{0}^{-1}.
    \end{equation}
\end{proof}
进一步可得
\begin{equation}
    \left(P^{+}(\zeta^{*}) \right)^{\dagger} M_{0} = M_{0} P^{-}(\zeta),
\end{equation}
\begin{proof}
    \begin{equation}
        \begin{aligned}
            (P^{+}(\zeta^{*}))^{\dagger} M_{0} & = \left(J_{+}(\zeta^{*}) H_{2} + J_{-}(\zeta^{*}) H_{1} \right)^{\dagger} M_{0} = H_{2} J_{+}^{\dagger}(\zeta^{*}) M_{0} + H_{1} J_{-}^{\dagger}(\zeta^{*}) M_{0} \\
                                               & = H_{2} M_{0} J_{+}^{-1}(\zeta) + H_{1} M_{0} J_{-}^{-1}(\zeta) = M_{0} \left(H_{2} J_{+}^{-1}(\zeta) + H_{1} J_{-}^{-1}(\zeta) \right) = M_{0} P^{-}(\zeta).
        \end{aligned}
    \end{equation}
\end{proof}
...
\begin{equation}
    \det P^{+} = \det(J_{+,1}, J_{+,2}, J_{-,3}) = \det J_{+} \times \begin{vmatrix}
        1 & 0 & e^{-2\theta} s_{13} \\ 0 & 1 & e^{-2\theta} s_{23} \\ 0 & 0 & s_{33}
...
\end{definition}
\begin{proof}
    考虑 $[J_{-}]_{1}$ 的 Neumann 级数有
    \begin{equation}
        c_{1} = \int_{-\infty}^{x} D_{1}(x,y,\zeta) Q(y,t) c_{0} \rmd y = \int_{-\infty}^{x} D_{1}(x,y,\zeta) (0,0,-r_{1}) \rmd y = \int_{-\infty}^{x} (0,0,-r_{1} e^{2 \rmi \zeta (y-x)}) \rmd y
    \end{equation}
    由 Riemann-Lebesuge 引理, $r_{1} \in L_{1}(\mathbb{R})$, 则 $c_{1} \to 0, \zeta \to \infty$. 同理可得 $c_{n} \to 0, \zeta \to \infty$. 又由 $\|c_{n} \| \leq \omega^{n}/n!$, 故极限和求和可交换
    \begin{equation}
        \lim_{\zeta \to \infty}[J_{-}]_{1} = \lim_{\zeta \to \infty} \sum_{n=0}^{\infty} c_{n} = \sum_{n=0}^{\infty} \lim_{\zeta \to \infty} c_{n} = (1,0,0)^{T}
    \end{equation}
    故可得$J_{\pm}$ 的分列渐近, $J_{-}^{-1}$ 的分行渐近, 故 \eqref{ch4-eq-RHP-normalization} 成立. 下证跳跃矩阵:
    \begin{equation}
        \begin{aligned}
...
\end{equation}
由 \eqref{ch4-eq-RHP-kernel} 可得 $\forall l \in 1, 2, \ldots, N$,
\begin{equation}
    \Gamma(\zeta_{l}) v_{l} = v_{l} - \sum_{j,k}^{N} v_{j} (M^{-1})_{jk} M_{kl} = 0, \quad
    \bar{v}_{l} \Gamma^{-1}(\bar{\zeta}_{l}) = \bar{v}_{l} - \sum_{j,k}^{N} M_{lj} (M^{-1})_{jk} \bar{v}_{k} = 0,
    \label{ch4-eq-Gamma-kernel}
\end{equation}
易见 $\Gamma \Gamma^{-1}$ 可能的极点仅为 $\zeta_{l}, \bar{\zeta}_{l}$, 且均为一阶极点. 由留数条件
\begin{equation}
    \Res_{\zeta = \zeta_{l}}(\Gamma \Gamma^{-1}) = - \Gamma v_{l} \sum_{k = 1}^{N} (M^{-1})_{lk} \bar{v}_{k} = 0, \quad \Res_{\zeta = \bar{\zeta}_{l}}(\Gamma \Gamma^{-1}) = \sum_{j = 1}^{N} v_{j} (M^{-1})_{jl} \bar{v}_{l} \Gamma^{-1}(\bar{\zeta}_{l}) = 0,
\end{equation}
由 \eqref{ch4-eq-Gamma-kernel} 可得: $\det \Gamma$ 在 $\zeta_{l}$ 处有简单零点, 在 $\bar{\zeta}_{l}$ 处有简单极点, 若记
\begin{equation}
    h = \det \Gamma(\zeta) \prod_{l=1}^{N} \frac{\zeta - \bar{\zeta}_{l}}{\zeta - \zeta_{l}},
\end{equation}
则 $h$ 恰好消去所有可能的零点和极点, 故为整函数. 由 $\Gamma \to I, \zeta \to \infty$ 可得 $h \to 1$, 由 Liouville 定理, $h \equiv 1$, 故
\begin{equation}
    \det \Gamma(\zeta) = \prod_{l=1}^{N} \frac{\zeta - \zeta_{l}}{\zeta - \bar{\zeta}_{l}}.
\end{equation}
定义 $\hat{P}^{+} = P^{+} \Gamma^{-1}, \quad \hat{P}^{-} = \Gamma P^{-}$, 易见它们在 $\zeta_{l}, \bar{\zeta}_{l}$ 处无奇点, 且满足
\begin{equation}
...
且
\begin{equation}
    \det\hat{P}^{+} = \det P^{+} \det \Gamma^{-1} = s_{33}(\zeta) \prod_{k = 1}^{N} \frac{\zeta - \bar{\zeta}_{k}}{\zeta - \zeta_{k}},  \quad
    \det\hat{P}^{-} = \hat{s}_{33}(\zeta) \prod_{k = 1}^{N} \frac{\zeta - \zeta_{k}}{\zeta - \bar{\zeta}_{k}}.
\end{equation}
在各自零点取极限可得
\begin{equation}
    \det \hat{P}^{+}(\zeta_{l}) = s_{33}'(\zeta_{l}) (\zeta_{l} - \bar{\zeta}_{l}) \prod_{k \neq l} \frac{\zeta_{l} - \bar{\zeta}_{k}}{\zeta_{l} - \zeta_{k}} \neq 0, \quad
    \det \hat{P}^{-}(\bar{\zeta}_{l}) = \hat{s}_{33}'(\bar{\zeta}_{l}) (\bar{\zeta}_{l} - \zeta_{l}) \prod_{k \neq l} \frac{\bar{\zeta}_{l} - \zeta_{k}}{\bar{\zeta}_{l} - \bar{\zeta}_{k}} \neq 0.
\end{equation}
其中 $s_{33}'(\zeta_{l})$ 表示在 $\zeta = \zeta_{l}$ 处的导数. 故 $\hat{P}^{\pm}$ 在 $\mathbb{C}_{\pm}$ 中解析无奇点, 连续到实轴, 且满足
\begin{equation}
    \begin{aligned}
        \hat{P}^{\pm} \to I, \quad \zeta \to \infty, \quad \zeta \in \mathbb{C}_{\pm}, \\
        \hat{P}^{-} \hat{P}^{+} = \Gamma G \Gamma^{-1} := \hat{G}, \quad \zeta \in \mathbb{R}.
    \end{aligned}
\end{equation}
由上式可得 $I - \hat{G} = \Gamma (I - G) \Gamma^{-1} \in L^{2}(\mathbb{R}) \cap L^{\infty}(\mathbb{R})$. 由正则 RH 问题的唯一性可得 $\hat{P}^{\pm}$ 唯一存在, 同样有 Plemelj 公式
\begin{equation}
...
对 $P^{+}$ 利用 Taylor 展开
\begin{equation}
    P^{+}(x,t,\zeta) = I + \frac{1}{\zeta} P_{1}^{+}(x,t) + O(\zeta^{-2}), \quad \zeta \to \infty
\end{equation}
将其带入 \eqref{ch4-eq-Lax-mod-x} 比较 $\zeta^{0}$ 的系数可得
\begin{equation}
...
为找到孤子解, 考虑无反射势情形, 即令 $G = I $, 有 $\hat{G} = I$. 此时 $\hat{P}^{-} = I, (\zeta \in \mathbb{C}_{-})$, 和 $\hat{P}^{+} = I, (\zeta \in \mathbb{C}_{+})$, 则 $P^{+} = \Gamma$, $P^{-} = \Gamma^{-1}$, 故
\begin{equation}
    ...
    \label{ch4-eq-RHP-nonreflection}
\end{equation}
为找出 $ v_{k}(x,t, \zeta) $, 将 $ P^{+}v_{k} = 0 $ 带入 \eqref{ch4-eq-Lax-mod-x}-\eqref{ch4-eq-Lax-mod-t} 得
\begin{equation}
    \begin{aligned}
    ...
    \implies
    \begin{aligned}
        P^{+}(\zeta_{k},x) (\frac{\partial v_{k}}{\partial x}+ \rmi \zeta_{k} \Lambda v_{k}) = 0, \\
        P^{+}(\zeta_{k},x) (\frac{\partial v_{k}}{\partial t} - 2 \rmi \zeta_{k}^{2} \Lambda v_{k}) = 0,
    \end{aligned}
    \implies
    \begin{aligned}
...
\section{逆空间约化情形}
或许读者会问, 相较于上一章节 $2 \times 2$ 在原方程做变换, 对 $3 \times 3$ 情形在原方程和伴随问题下做变换, 并直接取等价是否正确? 我们将以逆空间情形为例, 利用传统 RH 方法进行求解, 并最后证明二者的等价性, 然后在下节直接给出三个定理及简要证明.
...
\begin{align}
    P_{o}^{+} & = ([\phi_{+}]_{1}, [\phi_{+}]_{2}, [\phi_{-}]_{3}) E^{-1} = ([J_{+}]_{1}, [J_{+}]_{2}, [J_{-}]_{3}) = J_{+} H_{2} + J_{-} H_{1}, \quad \zeta \in \mathbb{C}_{+}, \\
    P_{o}^{-} & = ([\phi_{-}]_{1}, [\phi_{-}]_{2}, [\phi_{+}]_{3}) E^{-1} = ([J_{-}]_{1}, [J_{-}]_{2}, [J_{+}]_{3}) = J_{-} H_{2} + J_{+} H_{1}, \quad \zeta \in \mathbb{C}_{-}
\end{align}
...
则
\begin{align}
    P_{a}^{+} & = E ([\hat{\phi}_{-}]_{1}, [\hat{\phi}_{-}]_{2}, [\hat{\phi}_{+}]_{3})^{T} = \left([J_{-}^{-1}]_{1}, [J_{-}^{-1}]_{2}, [J_{+}^{-1}]_{3}\right)^{T} = H_{2} J_{-}^{-1} + H_{1} J_{+}^{-1}, \quad \zeta \in \mathbb{C}_{+}, \\
    P_{a}^{-} & = E ([\hat{\phi}_{+}]_{1}, [\hat{\phi}_{+}]_{2}, [\hat{\phi}_{-}]_{3})^{T} = \left([J_{+}^{-1}]_{1}, [J_{+}^{-1}]_{2}, [J_{-}^{-1}]_{3}\right)^{T} = H_{2} J_{+}^{-1} + H_{1} J_{-}^{-1}, \quad \zeta \in \mathbb{C}_{-}.
\end{align}
易见如下结论:
...
\begin{proof}
    ...
    \begin{equation}
        ...
        \end{aligned}
    \end{equation}
    与 \eqref{ch4-eq-reverse-x-Lax-adjoint} 作比较, 可得 $J_{+}^{\dagger}(-x,t,-\zeta^{*}) M_{1} = C J_{-}^{-1}(x,t,\zeta)$, 其中 $C$ 为常数矩阵, 由边界条件可取 $C = M_{1}$, 则
    \begin{equation}
...
同样的, 我们仍可得到类似于 \eqref{ch4-eq-RHP-kernel} 的对称关系, 但请注意其对称性仅限 $P_{o}^{\pm}$ 与 $P_{a}^{\pm}$, 仅限于同半平面,
\begin{equation}
    \begin{aligned}
...
\iffalse
    ...
    同理 $\bar{v}_{k} = \bar{v}_{k0} E^{-1}(\bar{\zeta}_{k}) = \bar{v}_{k} e^{\bar{\theta}_{k}\Lambda}$, 其中 $\bar{\theta} = -\rmi \bar{\zeta} x + 2\rmi \bar{\zeta}^{2} t$, $\bar{v}_{k0} = (\bar{\alpha}_{k}, \bar{\beta}_{k}, \bar{\gamma}_{k})$ 为常向量.
\fi
...
现在我们考虑同半平面关系标准无反射公式 \eqref{ch4-eq-RHP-nonreflection} 中 $P^{+}$ 的列核 $v_{k}$ 与 $P^{-}$ 的行核 $\bar v_{j}$ 的关系.

由于逆空间约化不联系 $\mathbb{C}_{\pm}$, 一般不能将 $\bar v_{j}$ 逐项写成 $v_{j}$ 的某种形式. 离散向量的正确限制应从重构势满足非局域约化这一条件得到, 考虑 $P_{1}^{+}$ 的非对角元
\begin{equation}
...
\begin{equation}
    \begin{aligned}
        C_{12}(x,t) & = \sum_{j,k=1}^{N} e^{-\theta_{j}} (\alpha_{j}, \beta_{j})^{T} (M^{-1})_{jk} \bar{\gamma}_{k}  e^{\bar{\theta}_{k}}  \\
        C_{21}(x,t) & = \sum_{j,k=1}^{N} e^{\theta_{j}} \gamma_{j} (M^{-1})_{jk} (\bar{\alpha}_{k}, \bar{\beta}_{k}) e^{-\bar{\theta}_{k}}
    \end{aligned}
\end{equation}
由 $Q = \rmi [\Lambda, P_{1}^{+}]$ 可得
\begin{equation}
...
将上式联立可得如下关系
\begin{equation}
    \sum_{j,k=1}^{N} \gamma_{j} e^{\theta_{j} + \theta_{k}} (M^{-1})_{jk}(x,t) (\bar{\alpha}_{k}, \bar{\beta}_{k}) =
    -\sum_{j,k=1}^{N} \gamma_{j}^{*} e^{\bar{\theta}_{k} + \theta_{j}} (M^{-1})_{jk}^{*}(-x,t) (\alpha_{k}^{*}, \beta_{k}^{*})^{T} A
    %- \left(\sum_{j,k=1}^{N} (\alpha_{j}(-x), \beta_{j}(-x))^{T} (M^{-1})_{jk}^{\dagger}(-x,t) \bar{\gamma}_{k} \right)^{\dagger} A.
...
\begin{equation}
    \begin{aligned}
        C_{12}(x,t) & = e^{-(\theta_{1}+\bar{\theta}_{1})} \vec{a}_{1} M_{11}^{-1} = \frac{\rmi (\bar{\eta}_{1} - \eta_{1})}{D_{11}} \vec{a}_{1} e^{-(\theta_{1}+\bar{\theta}_{1})}, \\
        C_{21}(x,t) & = e^{\theta_{1}+\bar{\theta}_{1}} \vec{b}_{1} M_{11}^{-1} = \frac{\rmi (\bar{\eta}_{1} - \eta_{1})}{D_{11}} \vec{b}_{1} e^{\theta_{1}+\bar{\theta}_{1}}.
    \end{aligned}
\end{equation}
将上式带入 \eqref{ch4-eq-reverse-x-discrete-spectrum-relation} 可得
\begin{equation}
    \begin{aligned}
        -C_{12}^{\dagger}(-x)A & = \frac{\rmi(\bar{\eta}_{1} - \eta_{1})}{D_{11}^{*}(-x)}\vec{a}_{1}^{\dagger}e^{\theta_{1}+\bar{\theta}_{1}} A_{1} \\
        C_{21}(x)              & = \frac{\rmi (\bar{\eta}_{1} - \eta_{1})}{D_{11}(x)} \vec{b}_{1} e^{\theta_{1}+\bar{\theta}_{1}}
    \end{aligned}
    \implies \frac{\vec{b}_{1}}{D_{11}(x)} = \frac{\vec{a}_{1}^{\dagger} A_{1}}{D_{11}^{*}(-x)}.
\end{equation}
记 $z = e^{\theta_{1} - \bar{\theta}_{1}}$, 则 $D_{11}(x) = \rho_{11} z^{-1} + z, D_{11}^{*}(-x) = \rho_{11}^{*} z + z^{-1}$, 则有
\begin{equation}
    \vec{b}_{1} (\rho_{11}^{*} z + z^{-1}) =\vec{a}_{1}^{\dagger} A_{1} (\rho_{11} z^{-1} + z)
\end{equation}
比较 $z$ 的次数有
\begin{equation}
    \begin{aligned}
        \vec{b}_{1} \rho_{11}^{*} & =\vec{a}_{1}^{\dagger}A_{1},          \\
        \vec{b}_{1}               & =\vec{a}_{1}^{\dagger}A_{1} \rho_{11}
...
\begin{equation}
    ...
    \frac{1}{\rho_{11}^{*}} \vec{a}_{1}^{\dagger} A_{1} \vec{a}_{1} \implies\vec{a}_{1}^{\dagger}A_{1} \vec{a}_{1} = 1.
\end{equation}
则有
\begin{equation}
    \vec{a}_{1}^{\dagger} A_{1} \vec{a}_{1} = 1, \quad \vec{b}_{1} = \rho_{11} \vec{a}_{1}^{\dagger} A_{1}, \quad |\rho_{11}| = 1
\end{equation}
...
\begin{equation}
    \begin{aligned}
        ...
        M_{jk}^{*}(-x, t, -\zeta_{k}^{*}) & = \frac{(\vec{b}_{j}^{*} E^{*}(-x, t, -\zeta_{j}^{*})) (E^{-1}(-x, t, -\zeta_{k}^{*}))^{*} (\vec{a}_{k}^{*}, 1)^{T}}{(-\zeta_{j}^{*} + \zeta_{k}^{*})^{*}}                                                                                                           \\
                                          & = \frac{\rho_{jk}^{*} e^{-(\theta(\bar{\zeta}_{j}) + \theta(\zeta_{k}))} + e^{\theta(\bar{\zeta}_{j}) + \theta(\zeta_{k})}}{-(\bar{\zeta}_{j} - \zeta_{k})}.
    \end{aligned}
\end{equation}
易见若设 $|\rho_{jk}| = 1$, 则有
\begin{equation}
    M_{jk}^{*}(-x,t, -\zeta_{k}^{*}) = \frac{e^{\theta(\bar{\zeta}_{j}) + \theta(\zeta_{k})} + \rho_{jk}e^{-( \theta(\bar{\zeta}_{j}) + \theta(\zeta_{k})}}{-\rho_{jk}(\bar{\zeta}_{j} - \zeta_{k})} = - \frac{1}{\rho_{jk}} M_{jk}(x,t,\zeta_{k})
\end{equation}
进一步, 若设
\begin{equation}
    v_{k0} = v_{0} = (\alpha, \beta, 1)^{T}, \quad \bar{v}_{j0} = \bar{v}_{0} = (\bar{\alpha}, \bar{\beta}, 1), \quad \forall j, k \in 1, 2, \dots N
\end{equation}
此时 $\rho_{jk} = \rho_{0} = \bar{\alpha} \alpha + \bar{\beta} \beta$, 且 $|\rho_{0}| = 1$, 则有
\begin{equation}
    M_{jk}^{*}(-x,t, -\zeta_{k}^{*}) = - \frac{1}{\rho_{0}} M_{jk}(x,t,\zeta_{k}) \implies M^{-1}(-x,t,-\zeta_{k}^{*}) = - \rho_{0} M^{-1}(x,t,\zeta_{k}),
\end{equation}
带入
\begin{equation}
    \begin{aligned}
        \begin{pmatrix}
            p^{*}(-x,t) \\ q^{*}(-x,t)
        \end{pmatrix} & = 2 \rmi C_{12}^{*}(-x,t,-\zeta_{k}^{*}) = 2 \rmi \sum_{j,k=1}^{N}\left( e^{-(\theta_{j} + \bar{\theta}_{k})} \vec{a}_{j0} (M^{-1})_{jk}(-x,t,-\zeta_{k}^{*}) \right)^{*} \\
                                   & = - 2 \rmi \rho_{0} \sum_{j,k=1}^{N} e^{-(\theta_{j} + \bar{\theta}_{k})} \vec{a}_{j0}^{*} (M^{-1})_{jk}
    \end{aligned}
\end{equation}
再利用 \eqref{ch4-eq-reverse-x-discrete-spectrum-relation} 可得
\begin{equation}
    (r_{1}, r_{2}) = - (p^{*}(-x,t), q^{*}(-x,t)) A_{1} = 2 \rmi \rho_{0} \sum_{j,k=1}^{N} e^{-(\theta_{j} + \bar{\theta}_{k})} (M^{-1})_{jk} \vec{a}_{j0}^{\dagger} A_{1}
\end{equation}
则有
\begin{equation}
    \vec{b}_{j0} = \vec{a}_{j0}^{\dagger} A_{1} \implies \bar{v}_{k0} = v_{k0}^{\dagger} M_{0}.
\end{equation}
且满足 $\bar{v}_{0} v_{0} = 1+\rho_{0}$.
...
\begin{equation}
    \begin{aligned}
        P^{+}(\zeta_{k}) v_{k} & = ([\phi_{+}]_{1}, [\phi_{+}]_{2}, [\phi_{-}]_{3}) E^{-1} v_{k} = ([\phi_{+}]_{1}, [\phi_{+}]_{2}, [\phi_{-}]_{3}) E^{-1} E v_{k0} = 0, \\
                               & = \alpha_{k} [\phi_{+}]_{1} + \beta_{k} [\phi_{+}]_{2} + \gamma_{k} [\phi_{-}]_{3} = 0.
    \end{aligned}
    \label{ch4-eq-reverse-x-phi-linear-combination}
\end{equation}
其中
\begin{equation}
    [\phi_{+}]_{1} \sim (e^{\rmi \zeta_{k} x}, 0,0)^{T}, \quad [\phi_{+}]_{2} \sim (0, e^{\rmi \zeta_{k} x}, 0)^{T}, \quad x \to +\infty, \quad
    [\phi_{-}]_{3} \sim (0,0, e^{- \rmi \zeta_{k} x})^{T}, \quad x \to -\infty
\end{equation}
又由于
\begin{equation}
...
\begin{equation}
    Y_{k} = \alpha_{k}' [\phi_{+}]_{1} + \beta_{k}' [\phi_{+}]_{2} = -\gamma_{k}' [\phi_{-}]_{3}
\end{equation}
则有
\begin{equation}
    \alpha_{k}' [\phi_{+}]_{1} + \beta_{k}' [\phi_{+}]_{2} + \gamma_{k}' [\phi_{-}]_{3} = 0
...
现在我们可以对问题 \ref{ch3-question-RH-and-eigenvalue-problem} 做肯定的回答. 利用特征值问题得到的对称关系和 RH 方法得到的对称关系是相同的, 而且不但可以对原问题求对称性, 也可同时联立原问题和伴随问题求解对称性.
...
\begin{theorem}[逆空间] \label{ch4-thm:inverse-space}
    对于逆空间 NLS 方程\eqref{ch4-eq-reduction-inverse-x-p}-\eqref{ch4-eq-reduction-inverse-x-q}, 如果 $ \zeta $ 是一个特征值, 则 $ -\zeta^{*} $ 也是一个特征值. 因此，非纯虚特征值总是以 $ (\zeta, -\zeta^{*}) $ 的形式成对出现，它们位于复平面的同一半平面。对称关系如下:
    \begin{equation}
        C_{21}(x,t) = - C_{12}^{\dagger}(-x,t) A_{1}.
    \end{equation}
    特别的, 若设
    \begin{equation}
        v_{j0} = v_{0} = (\vec{a}, 1)^{T}, \quad \bar{v}_{k0} = \bar{v}_{0} = (\vec{b}, 1), \quad \forall j, k \in 1, 2, \dots N
    \end{equation}
    则满足 $\vec{b} = \rho \vec{a}^{\dagger} A_{1}$, 且 $\bar{v}_{k0} v_{j0} = 1 + \rho_{0}$, 其中 $\rho_{0} = \vec{b} \vec{a}$, 且 $|\rho_{0}| = 1$.

    进一步当上下半平面各包含一个纯虚特征值时. 必有
    \begin{equation}
        \zeta_{1} = \rmi \eta_{1} \in \rmi \mathbb{R}_{+}, \quad \bar{\zeta}_{1} = \rmi \bar{\eta}_{1} \in \rmi \mathbb{R}_{-}, \quad v_{10} = (\vec{a}_{1}, 1)^{T}, \quad \bar{v}_{10} = (\vec{b}_{1}, 1),
    \end{equation}
    满足
    \begin{equation}
        \vec{b}_{1} = \rho_{11} \vec{a}_{1}^{\dagger} A_{1}, \quad \vec{a}_{1}^{\dagger} A_{1} \vec{a}_{1} = 1, \quad |\rho_{11}| = 1.
    \end{equation}
\end{theorem}
在证明开始之前, 我们仿照上一章节, 先建立离散数据之间的关系.

考虑离散数据 $ \{\zeta_{k}, \bar{\zeta}_{k}, \alpha_{k}, \beta_{k}, \bar{\alpha}_{k}, \bar{\beta}_{k}, \gamma_{k}, \bar{\gamma}_{k}, 1 \leq k \leq N\} $ 及原谱问题 $Y_{x} = MY$ 和伴随谱问题 $K_{x} = -KM $
\begin{align}
...
\begin{align}
    K_{k}(x) & \to \left(\bar{\alpha}_{k} \rme^{-\rmi \bar{\zeta}_{k} x},\bar{\beta}_{k} \rme^{-\rmi \bar{\zeta}_{k} x}, 0\right), \quad x \to +\infty, \\
    K_{k}(x) & \to \left(0, 0, -\bar{\gamma}_{k}\rme^{\rmi \bar{\zeta}_{k}x}\right), \quad x \to -\infty
    \label{eq:large-x-asymptotics-of-reverse-x-3d-NLS-adjoint-eigenfunction}
\end{align}
...
\begin{proof}
    此时 $ r_{1} = a p^{*}(-x) + b^{*} q^{*}(-x), r_{2} = b^{*} p^{*}(-x) + d q^{*}(-x) $, 容易发现 $ Q^{\dagger}(-x) = M_{1} Q M_{1}^{-1} $, 所以我们对 \eqref{eq:3-NLS-eigenvalue-problem} 式先共轭转置, 再进行变量替换 $ x \to -x $, 则有
    \begin{equation}
        Y_{x} = - \rmi \zeta \Lambda Y + Q Y \implies -Y_{x}(-x)  = - \rmi \zeta \Lambda Y(-x) + Q(-x) Y(-x)
    \end{equation}
    两边同时做 Hermitian 转置有
    \begin{equation}
        -Y_{x}^{\dagger}(-x) = \rmi \zeta^{*} Y^{\dagger}(-x) \Lambda + Y^{\dagger}(-x) Q^{\dagger}(-x)
    ...
    其中
    \begin{equation}
        \hat{Y}(x) = Y(-x)^{\dagger}M_{1}, \quad \hat{\zeta} = -\zeta^{*}, \label{ch4-eq-reverse-x-symmetry-parameters}
    \end{equation}
...
\end{theorem}
\begin{proof}
    显然有 $ Q^{T}(x,-t) = -M_{2} Q(x,-t) M_{2}^{-1} $, 对 \eqref{eq:3-NLS-eigenvalue-problem} 同时转置, 对 Lax 对做替换 $t \to -t$ 得并转置得
    \begin{equation}
        Y_{x}^{T}(x,-t) = -\rmi \zeta Y^{T}(x,-t) \Lambda + Y^{T}(x,-t) Q^{T}(x,-t) = \rmi \zeta Y^{T}(x,-t) \Lambda - Y^{T}(x,-t) M_{2} Q(x,t) M_{2}^{-1}
    \end{equation}
    两边同右乘 $M_{2}$ 有
    \begin{equation}
        Y_{x}^{T}(x,-t) M_{2} = -\rmi \zeta Y^{T}(x,-t) \Lambda M_{2} - Y^{T}(x,-t) M_{2} Q(x,-t)
    \end{equation}
    则有
    \begin{equation}
        \hat{Y}_{x}(x,t) = -\hat{Y}(x,t)(\rmi \hat{\zeta}\Lambda + Q(t)), \quad \hat{Y}(t) = Y(-t)^{T}M_{2}, \quad \hat{\zeta} = -\zeta
    \end{equation}
...
\end{proof}
\begin{remark}
    注意逆时间得到的对称关系为 $\hat{\zeta} = -\zeta$, 联系了 $C_{\pm}$ 的特征值, 这是其与逆空间, 逆时空最大的不同.
\end{remark}
...
\begin{theorem}[逆时空] \label{ch4-thm:inverse-space-time}
    ...
    则其对应的特征向量满足
    \begin{equation}
        \vec{b} = \rho_{jk}\vec{a}^{T} A_{2}, \quad \bar{v}_{0} v_{0} = 1+ \rho_{0}
        \label{eq:3-NLS-reverse-x-t-eigenvector-symmetry}
    \end{equation}
    其中 $\rho_{0} = \vec{b} \cdot \vec{a} = \pm 1$ 为常数.
\end{theorem}
\begin{proof}
    对于逆时空方程情形, 易见 $ Q^{T}(-x,-t) = M_{3} Q M_{3}^{-1} $, 则
    \begin{equation}
        \begin{aligned}
    ...
    \begin{equation}
        M_{jk} = \frac{\rho_{jk} e^{\bar{\theta}_{j} - \theta_{k}} + e^{-(\bar{\theta}_{j} - \theta_{k})}}{\bar{\zeta}_{j} - \zeta_{k}}, \quad
        M_{jk}(-x,-t) = \frac{\rho_{jk} e^{-(\bar{\theta}_{j} - \theta_{k})} + e^{\bar{\theta}_{j} - \theta_{k}}}{\bar{\zeta}_{j} - \zeta_{k}},
    \end{equation}
    故若 $\rho_{jk} = \vec{b} \vec{a} = \pm 1$, 则有 $M_{jk}(-x,-t) = \rho_{jk} M_{jk}$. 此时若对所有 $j, k$ 取定 $\rho_{jk} = \rho_{0} = \pm 1$, 有 $M(-x,-t) = \pm M$, 则有
    \begin{equation}
        (p(-x,-t), q(-x,-t))^{T} = -2 \rmi C_{12} = - 2 \rmi \rho_{0} \sum_{j,k=1}^{N} e^{\theta_{k} + \bar{\theta}_{j}} (M^{-1})_{jk} \vec{a}_{j}
    \end{equation}
    利用 $Q$ 的约化关系有
    \begin{equation}
        (r_{1}, r_{2}) = (p(-x,-t), q(-x,-t)) A_{2} = - 2 \rmi \rho_{0} \sum_{j,k=1}^{N} e^{\theta_{k} + \bar{\theta}_{j}} (M^{-1})_{jk} \vec{a}_{j} A_{2}
    \end{equation}
    若设定
    \begin{equation}
        v_{j0} = (\vec{a}_{j}, 1)^{T}, \quad \bar{v}_{k0} = (\vec{b}_{k}, 1), \quad \forall j, k \in 1, 2, \dots N
    \end{equation}
    此时有 $\vec{b} = \rho \vec{a} A_{2}$.
\end{proof}
和上一章节的三个定理显示: 相比于 $ 2 \times 2 $ AKNS 系统导出的 NLS 方程, $ 3\times 3 $ 没有那么好的性质, 只能使用转置和共轭转置的方式求解, 尤其是逆空间和逆时空情形相比二阶的性质更差, 这是因为二阶矩阵存在 Pauli 矩阵基底, 而三阶矩阵不存在类似的完备基底.

\subsection{三种约化下解的动力学分析}
逆时空作为限制最严格得到的解, 同时也具有最好的性质, 可移动, 可坍缩, 可以展现出大部分局部方程的性质, 因此本小节我们讨论一下它的性质,
\subsubsection{逆时空}
考虑函数
\begin{equation}
    \begin{aligned}
...
\end{align}
则若 $ \Re(\omega_{1}) \neq 0 $, 则解在 $ t \to \pm \infty $ 时爆炸; 若 $ \Re(\omega_{2}) \neq 0 $, 则解会周期性的坍塌.
\subsection{原因}
占位
\section{进一步考虑二重约化情形}
本节考虑 $ p, q $ 的约化关系, 占位,
