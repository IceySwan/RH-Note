...
\end{definition}
\begin{proof}
    由 $P^{-} = ([J_{-}]_{1}, [J_{-}]_{2}, [J_{+}]_{3})$, 考虑 $[J_{-}]_{1}$ 的 Neumann 级数有
    \begin{equation}
    ...
    \end{equation}
    同理可得$[J]_{-}]_{2}, [J_{+}]_{3}$, 故 \eqref{ch4-eq-RHP-normalization} 成立. 下证跳跃矩阵:
    \begin{equation}
        \begin{aligned}
...
\begin{equation}
    ...
    \det \hat{P}^{-}(\zeta_{l}) = \hat{s}_{33}'(\zeta_{l}) (\zeta_{l} - \bar{\zeta}_{l}) \prod_{k \neq l} \frac{\zeta_{l} - \zeta_{k}}{\zeta_{l} - \bar{\zeta}_{k}} \neq 0.
\end{equation}
...
\begin{align}
    P_{o}^{+} & = (\phi_{+,1}, \phi_{+,2}, \phi_{-,3}) E^{-1} = ([J_{+}]_{1}, [J_{+}]_{2}, [J_{-}]_{3}) = J_{+} H_{2} + J_{-} H_{1}, \quad \zeta \in \mathbb{C}_{+}, \\
    P_{o}^{-} & = (\phi_{-,1}, \phi_{-,2}, \phi_{+,3}) E^{-1} = ([J_{-}]_{1}, [J_{-}]_{2}, [J_{+}]_{3}) = J_{-} H_{2} + J_{+} H_{1}, \quad \zeta \in \mathbb{C}_{-}
\end{align}
...
\begin{proof}
    ...
    \begin{equation}
        ...
        \end{aligned}
    \end{equation}
    与 \eqref{ch4-eq-reverse-x-Lax-adjoint} 作比较, 可得 $J_{+}^{\dagger}(-x,t,-\zeta^{*}) M_{1} = C J_{-}^{-1}(x,t,\zeta)$, 其中 $C$ 为常数矩阵, 由边界条件, 不失一般性可取 $C = M_{1}$, 则
    \begin{equation}
...
将上式联立可得如下关系
\begin{equation}
    \sum_{j,k=1}^{N} \gamma_{j} (M^{-1})_{jk}(x,t) (\bar{\alpha}_{k}, \bar{\beta}_{k}) = - \left(\sum_{j,k=1}^{N} (\alpha_{j}(-x), \beta_{j}(-x))^{T} (M^{-1})_{jk}^{\dagger}(-x,t) \bar{\gamma}_{k} \right)^{\dagger} A.
...
\begin{remark}
    这里写为 $D_{11}^{*}(-x)$ 是因为 $\zeta_{1} = -\zeta_{1}^{*}$ 为纯虚数, 故省略. 对于多孤子情形, 存在 $\zeta_{k} = a+b \rmi$ 情形, 故必须写明 $D_{11}(x,t,\zeta_{1})$ 和 $D_{11}^{*}(-x,t,-\zeta_{1}^{*})$ 的关系
\end{remark}
...
易见若设 $|\rho_{jk}| = 1$, 则有
\begin{equation}
    M_{jk}(-x,t, -\zeta^{*}) = \frac{e^{\theta(\bar{\zeta}_{j}) + \theta(\zeta_{k})} + e^{-( \theta(\bar{\zeta}_{j}) + \theta(\zeta_{k})}}{-(\bar{\zeta}_{j} - \zeta_{k})} \implies \rho_{jk} M_{jk} (-x, t, -\zeta^{*}) = - M_{jk}^{*},
\end{equation}
则有
\begin{equation}
    \begin{aligned}
...
\begin{theorem}[逆空间] \label{ch4-thm:inverse-space}
    ...
    \end{equation}
    则满足 $\bar{v}_{j0} = v_{k0}^{\dagger} M_{0}$, 且 $\bar{v}_{j0} v_{j0} = 1 + \rho$, 其中 $\rho = \alpha \bar{\alpha} + \beta \bar{\beta}$, 且 $|\rho| = 1$.
...
\end{proof}
\begin{remark}
    注意逆时间与其余两种约化最大的不同为: 逆时间得到的特征值为相对半平面对称关系, 故可直接得到 $\zeta_{k}, v_{k}$ 与 $\bar{\zeta}_{k}, \bar{v}_{k}$ 的关系因逆时间与上一章节相同, 在此略过具体分析.
\end{remark}
...
\begin{theorem}[逆时空] \label{ch4-thm:inverse-space-time}
    ...
    \end{equation}
    其中 $\rho = \rho = \alpha \bar{\alpha} + \beta \bar{\beta} = \pm 1$ 为常数.
\end{theorem}
\begin{proof}
